...
var InstructionSet = function (params) {
  ...
  var conditionalInvariants = params.conditionalInvariants;
  ...
  self.pop = function (symbol) {
    ...
    conditionalInvariants[symbol] = conditionalInvariants[stackSymbol];
  ...
  self.fetch = function (keySymbol, skipBoundsCheck) {
    keySymbol = keySymbol || stack.pop();
    var arraySymbol = stack.pop();

    checkFetchTypes(keySymbol, arraySymbol);
    applyConditionalInvariants(keySymbol, arraySymbol);

    if (!skipBoundsCheck) {
      applyBoundsInvariants(keySymbol, arraySymbol);
    }
    ...
    if (elementType === "array") {
      ...
      for (var i = 0; i < maxWidth; i += 1) {
        ...
        _.each(nestedArrays, function (nestedArray, arrayIndex) {
          ...
          if (nestedSymbol) {
            ...
            var invariantSymbol = registry.nextBoolean()[0];
            ...
            codeWriter.instruction({ type: "not" });
            codeWriter.instruction({ type: "pop", symbol: invariantSymbol });

            invariants.push({
              invariantSymbol: invariantSymbol,
              conditionIndex: i
            });
          }
        });
      ...
      _.each(arraySymbols, function (symbol, i) {
        var existingInvariants = conditionalInvariants[symbol];
        existingInvariants = existingInvariants || [];

        _.each(existingInvariants, function (inv) {
          var invariantSymbol = registry.nextBoolean()[0];

          codeWriter.instruction({ type: "push", symbol: keySymbols[0] });
          codeWriter.instruction({ type: "constant", value: inv.conditionIndex });
          codeWriter.instruction({ type: "equal" });
          codeWriter.instruction({ type: "not" });
          codeWriter.instruction({ type: "push", symbol: inv.invariantSymbol });
          codeWriter.instruction({ type: "or" });
          codeWriter.instruction({ type: "pop", symbol: invariantSymbol });
      ...
      conditionalInvariants[outputSymbol] = invariants;
  ...
  self.width = function () {
    ...
    if (arrayType !== "array") {
      throw new Error("Wrong type for width: " + arrayType);
    }
    ...
    var restrictions = conditionalInvariants[arraySymbol];
    restrictions = restrictions || [];

    var conditionalIndexes = 0;
    _.each(arraySymbols, function (s, index) {
      var indexRestrictions = _.select(restrictions, function (r) {
        return r.nilIndex === index
      });

      if (indexRestrictions.length > 0) {
        conditionalIndexes += 1;

        _.each(indexRestrictions, function (r) {
          codeWriter.instruction({ type: "push", symbol: r.keySymbols[0] });
          codeWriter.instruction({ type: "constant", value: r.keyIndex });
          codeWriter.instruction({ type: "equal" });
        });

        for (var i = 0; i < indexRestrictions.length - 1; i += 1) {
          codeWriter.instruction({ type: "or" });
        }
    ...
    var unconditionalIndexes = arraySymbols.length - conditionalIndexes;
    codeWriter.instruction({ type: "constant", value: unconditionalIndexes });

    for (var i = 0; i < conditionalIndexes; i += 1) {
      codeWriter.instruction({ type: "add" });
    }
  ...
  var checkFetchTypes = function (keySymbol, arraySymbol) {
  ...
  var applyConditionalInvariants = function (keySymbol, arraySymbol) {
    var invariants = conditionalInvariants[arraySymbol];
    var keySymbols = symbolTable.symbols(keySymbol);

    if (invariants) {
      _.each(invariants, function (inv) {
        codeWriter.instruction({ type: "push", symbol: keySymbols[0] });
        codeWriter.instruction({ type: "constant", value: inv.conditionIndex });
        codeWriter.instruction({ type: "equal" });
        codeWriter.instruction({ type: "not" });
  ...
  var applyBoundsInvariants = function (keySymbol, arraySymbol) {
    var keySymbols = symbolTable.symbols(keySymbol);
    var arraySymbols = symbolTable.symbols(arraySymbol);
